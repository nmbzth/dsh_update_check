...
6. **破坏性更新预警（重要）**：DSH 官方公告未来将有破坏性更新，可能与旧插件不兼容。插件双重信号检测破坏性更新——① 语义版本（major 变化，或 0.x 阶段 minor 变化）；② 官方发布说明关键词（高置信度措辞，如 breaking change / 破坏性更新 / 破坏…兼容 等）。命中时横幅**黄色高亮 + ⚠️**，必须经过「了解风险 → 我了解风险，确认更新」**二次确认**才会执行安装；设置页按钮同样两段式确认。
